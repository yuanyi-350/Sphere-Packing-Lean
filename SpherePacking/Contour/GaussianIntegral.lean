...
public lemma integral_const_mul_phase_gaussian_pi_mul_I_mul_even (k : ℕ)
  ...
  calc
    (∫ x : EuclideanSpace ℝ (Fin (2 * k)), c * (Complex.exp (↑(-2 * (Real.pi * ⟪x, w⟫)) * Complex.I) *
      Complex.exp ((Real.pi : ℂ) * Complex.I * ((‖x‖ ^ 2 : ℝ) : ℂ) * z))) =
      c * ∫ x : EuclideanSpace ℝ (Fin (2 * k)), Complex.exp (↑(-2 * (Real.pi * ⟪x, w⟫)) * Complex.I) *
        Complex.exp ((Real.pi : ℂ) * Complex.I * ((‖x‖ ^ 2 : ℝ) : ℂ) * z) := MeasureTheory.integral_const_mul c _
    _ = c * ((((Complex.I : ℂ) / z) ^ k) * Complex.exp ((Real.pi : ℂ) * Complex.I * (‖w‖ ^ 2 : ℝ) * (-1 / z))) := by
      rw [ForMathlib.integral_phase_gaussian_pi_mul_I_mul_even (k := k) (w := w) (z := z) hz]
