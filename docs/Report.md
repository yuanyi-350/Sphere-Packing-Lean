...
2. 将 `.lake` 里面的软连接共用, 避免占用磁盘空间爆炸
3. 拓扑排序, 从底层向上修 (现在已经设计了完善的并行算法) 
4. 按照拓扑排序的顺序, 每次选取一个文件, 让codex修改, 直到这个文件可编译.
5. 对于每个文件, 指示codex从上往下一个错一个错的修 
6. 继续下一个文件.
7. 所有文件处理结束后 `lake build` 并发邮件通知.



成果 : 

1. 挑选了why学长提供的仓库, 把 [README.md](../README.md) 里的仓库基本上完成了升级, 
2. 平均每个仓库花费不超过2块钱 (codex 便宜号池), 平均每个仓库 $\le 2M$ , 
3. 在九坤的服务器上跑通了工作流.

局限 : 

1. 仓库里不能有 `sorry` , 并且单个文件不要超过 1500 行
2. 偶尔遇上一些奇怪的问题需要手动修理, [例子](https://github.com/yuanyi-350/lean-stat-learning-theory/commit/f9a4c41688cdcfd21e371c80f93bb5510705402a)
...
5. Localy 修改 (如果要考虑给 lean-community 用的话)
